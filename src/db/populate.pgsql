...
\copy work.raw1 from '/home/bildoon/dev/loto/assets/loto-1976-2008.csv' with (format csv, delimiter ';', header true)
  ...
  insert into app.draws (
    rule_set, source_file, draw_ref, draw_sub,
    draw_date, draw_dow, draw_day,
    order_nums, main_sorted,
  ...
  select
    ...
    coalesce(
      work.normalize_draw_day(r.jour_de_tirage),
      case extract(isodow from d.draw_date)::int
        when 1 then 'lundi'
        when 2 then 'mardi'
    ...
    'm:' || btrim(r.combinaison_gagnante_en_ordre_croissant)
        || '|complementaire:' || btrim(r.boule_complementaire)
  from work.raw1 r
  cross join lateral (
    select to_date(btrim(r.date_de_tirage), 'yyyymmdd') as draw_date
  ) d
  where nullif(btrim(r.combinaison_gagnante_en_ordre_croissant), '') is not null
...
\copy work.raw234 from '/home/bildoon/dev/loto/assets/loto-2017-2019.csv' with (format csv, delimiter ';', header true)
  ...
  select
    ...
    array[
      nullif(btrim(r.boule_1_second_tirage), '')::int,
      nullif(btrim(r.boule_2_second_tirage), '')::int,
      nullif(btrim(r.boule_3_second_tirage), '')::int,
      nullif(btrim(r.boule_4_second_tirage), '')::int,
  ...
  where nullif(btrim(r.combinaison_gagnant_second_tirage_en_ordre_croissant), '') is not null
  on conflict (rule_set, draw_ref, draw_sub) do nothing;
